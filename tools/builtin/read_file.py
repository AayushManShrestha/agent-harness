...
class ReadFileParams(BaseModel):
    path: str = Field(..., description="Path to the file to read (relative or absolute path)")
    offset: int = Field(1, ge=1, description="Line number to start reading from (1-based). Defaults to 1.")
    limit: int | None = Field(None, ge=1, description="Maximum number of lines to read. If not specfied, reads entire file")

class ReadFileTool(Tool):
    name = "read_file"
    description = """
        Read the contents of a text file. Returnts the file content with line numbers.
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        if file_size > self.MAX_FILE_SIZE:
            return ToolResult.error_result(f"File too large ({file_size/(1024*1024):.1f}MB). Max size limit is {self.MAX_FILE_SIZE/(1024*1024):.0f}MB.")

        if is_binary_file(path):
            return ToolResult.error_result(f"Cannot read binary file: {path.name}. This tool only reads text files.")
        ...
        try:
            ...
            token_count = count_tokens(output, "poolside/laguna-s-2.1:free")
            truncated = False
